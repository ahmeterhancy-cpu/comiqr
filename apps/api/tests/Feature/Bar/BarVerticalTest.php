...
/** Seed a bar venue with a 100 TL product; $happy is merged into settings_json. */
function seedBar(array $happy = []): array
{
    $tenant = Tenant::factory()->create(['settings_json' => array_merge(['vertical' => 'bar'], $happy)]);
...
it('applies the happy-hour discount at order time (within the window)', function () {
    Carbon::setTestNow(Carbon::parse('2026-07-03 23:00')); // window 22:00–02:00
...
it('does not discount outside the happy-hour window', function () {
    Carbon::setTestNow(Carbon::parse('2026-07-03 15:00'));
    ['product' => $product, 'table' => $table] = seedBar([
        'happy_hour' => ['enabled' => true, 'start' => '22:00', 'end' => '02:00', 'percent' => 20],
    ]);
...
it('exposes the active happy-hour on the menu payload', function () {
    Carbon::setTestNow(Carbon::parse('2026-07-03 23:30'));
    ['table' => $table] = seedBar([
...
it('ignores a disabled happy-hour', function () {
    Carbon::setTestNow(Carbon::parse('2026-07-03 23:00'));
    ['product' => $product, 'table' => $table] = seedBar([
