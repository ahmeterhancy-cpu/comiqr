...
export function TrialCard() {
  ...
  const endDate = fmtDate(trialing ? st.trial_ends_at : sub?.current_period_end);
  ...
  if (pastDue) {
    ...
    return (
      <Link href="/billing" className="mt-4 block rounded-2xl border border-red-400/40 bg-red-500/10 p-3.5 transition hover:bg-red-500/15">
        ...
        <div className="text-[11px] text-red-200/90">Kredi kartınızdan tahsil edilemedi.</div>
        <div className="mt-2 rounded-lg bg-white/10 px-2.5 py-1.5 text-[11px] font-semibold text-white">
          {grace > 0 ? `Ek ödeme için ${grace} gün süreniz var` : 'Ek süre doldu — ödeme bekleniyor'}
        </div>
  ...
  return (
    <div className="mt-4 rounded-2xl bg-white/5 p-3.5">
      ...
      >
        {trialing ? 'Ücretsiz Deneme' : 'Aktif'}
      </span>
      <div className="mt-1.5 text-base font-extrabold text-white">{planName}</div>
      <div className="text-[11px] text-slate-400">{trialing ? 'Deneme sürümündesiniz' : 'Aboneliğiniz aktif'}</div>
